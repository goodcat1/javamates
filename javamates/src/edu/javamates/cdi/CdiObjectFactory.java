 ...
 * or Results, to be injected by a Contexts and Dependency Injection container
 * (JSR299 / WebBeans). The BeanManager instance will be searched in the
 * container's JNDI context, according to following algorithm:
 * <ul>
 * <li>if a value for configuration constant
 * <code>struts.objectFactory.cdi.jndiKey</code> is given, this key will be
 * looked up</li>
 * <li>if no BeanManager found so far, look under
 * {@link #CDI_JNDIKEY_BEANMANAGER_COMP}</li>
 * <li>if no BeanManager found so far, look under
 * {@link #CDI_JNDIKEY_BEANMANAGER_APP}</li>
 * <li>if no BeanManager found so far, look under
 * {@link #CDI_JNDIKEY_BEANMANAGER_COMP_ENV}</li>
 * </ul>
 */
public class CdiObjectFactory extends ObjectFactory {

	private static final long serialVersionUID = -5920067698018576931L;

	private static final Logger LOG = LoggerFactory
			.getLogger(CdiObjectFactory.class);
	...
	/**
	 * The key under which the BeanManager can be found according to JBoss Weld
	 * docs
	 */
	...
	public CdiObjectFactory() {
		...
		beanManager = findBeanManager();

		if (beanManager != null) {
			ctx = buildNonContextualCreationalContext(beanManager);
	...
	/**
	 * Try to find the CDI BeanManager from JNDI context. First, if provided,
	 * the key given by struts.objectFactory.cdi.jndiKey will be checked. Then,
	 * if nothing was found or no explicit configuration was given, the key
	 * {@link #CDI_JNDIKEY_BEANMANAGER_COMP} will be tested. If nothing is found
	 * there, the key {@link #CDI_JNDIKEY_BEANMANAGER_APP} will be checked. If
	 * still nothing is found there, the key
	 * {@link #CDI_JNDIKEY_BEANMANAGER_COMP_ENV} will be checked.
	 * 
	 ...
	 * @param jndiKeyToCheck
	 *            the key to lookup.
	 * @return the BeanManager, if found; <tt>null</tt> if not found or
	 *         {@link javax.naming.NamingException} was thrown.
	 */
	protected BeanManager lookup(Context context, String jndiKeyToCheck) {
		if (LOG.isInfoEnabled()) {
			LOG.info("[lookup]: Checking for BeanManager under JNDI key "
					+ jndiKeyToCheck);
		}
		...
		try {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("[lookup]: BeanManager lookup failed for JNDI key "
						+ jndiKeyToCheck, e);
			}
		}
	...
	/**
	 * Fill InjectionTarget cache for the class with the specified name
	 * 
	 * @param className
	 *            Class name
	 ...
	 *             Class not found
	 */
	public void fillInjectionTargetCache(String className)
			throws ClassNotFoundException {
		getInjectionTarget(getClassInstance(className));
	}

	/**
	 * Get a InjectionTarget instance for a given class. If the appropriate
	 * target is not found in cache, a nw instance will be created.
	 * 
	 * @param clazz
	...
	protected InjectionTarget<?> getInjectionTarget(Class<?> clazz) {
		...
		if (result == null) {
			result = beanManager.createInjectionTarget(beanManager
